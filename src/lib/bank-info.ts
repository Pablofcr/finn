...
export type BankInfo = {
  ...
  name: string
  /** Initials shown inside the avatar */
  monogram: string
  /** Brand color used as background */
  bgColor: string
  /** Foreground (text) color — usually white, sometimes black for yellow-ish brands */
  fgColor: string
...
export const BANKS: BankInfo[] = [
  ...
  { name: 'Visa', monogram: 'VISA', bgColor: '#1A1F71', fgColor: '#F7B600', keywords: ['visa'] },
  { name: 'Mastercard', monogram: 'MC', bgColor: '#1A1F71', fgColor: '#FFFFFF', keywords: ['mastercard', 'master card', 'master'] },
  { name: 'Elo', monogram: 'ELO', bgColor: '#000000', fgColor: '#FCEC32', keywords: ['elo'] },
  { name: 'Amex', monogram: 'AMEX', bgColor: '#006FCF', fgColor: '#FFFFFF', keywords: ['amex', 'american express'] },
  { name: 'Hipercard', monogram: 'HC', bgColor: '#B11E2C', fgColor: '#FFFFFF', keywords: ['hipercard'] },
  { name: 'Diners', monogram: 'DC', bgColor: '#0079BE', fgColor: '#FFFFFF', keywords: ['diners'] },

  // ─── Digital banks / fintechs ──────────────────────────────────────────────
  { name: 'Nubank', monogram: 'N', bgColor: '#820AD1', fgColor: '#FFFFFF', keywords: ['nubank', 'nu bank', 'nu pagamentos'] },
  ...
  { name: 'Mercado Pago', monogram: 'MP', bgColor: '#00B1EA', fgColor: '#FFFFFF', keywords: ['mercado pago', 'mercadopago'] },
  { name: 'PagBank', monogram: 'PB', bgColor: '#00BF63', fgColor: '#FFFFFF', keywords: ['pagbank', 'pagseguro', 'pag bank', 'pag seguro'] },
  { name: 'PicPay', monogram: 'P', bgColor: '#21C25E', fgColor: '#FFFFFF', keywords: ['picpay', 'pic pay'] },
...
export function detectBankFromName(accountName: string | null | undefined): BankInfo | null {
  ...
  const text = ` ${normalize(accountName)} ` // pad with spaces so word-boundary checks work
  for (const bank of BANKS) {
    for (const kw of bank.keywords) {
      const padded = ` ${kw} `
      if (text.includes(padded)) return bank
      // also accept the keyword at the very start or end
